...
from opparse.types import plist
from opparse.parse.nodes import *
from opparse.types import space, api
from opparse.misc import platform, strutil
...
def add_pattern(patterns, args):
    ...
    args[0] = (args[0])
    return plist.cons(plist.plist(args), patterns)
...
def _process_map(state, pattern, patterns, path):
    ...
    patterns = add_pattern(patterns, ["map", plist.plist(symbols), _create_path_node(pattern, path)])
...
def process_patterns(state, pattern, path, index):
    ...
    patterns = plist.cons(index, patterns)
